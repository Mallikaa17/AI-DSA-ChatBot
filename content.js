function createChatBot() {
  ...
  chatInput.placeholder = 'Ask your query';
  ...
  addMessage('bot', 'Ask your query', chatMessages);
...
async function sendToGemini(userQuestion, problemInfo, chatMessagesElement) {
  try {
    ...
    if (!API_KEY) {
      addMessage('bot', 'API key not found. Set your Gemini API key.', chatMessagesElement);
      return;
    }

    // Fix: Safely get a key from localStorage and handle potential null values
    let UserId = "unknown";
    try {
      ...
      // Continue with default "unknown" UserId
    }

    function extractQuestionNumber(url) {
      // Fix: Check if url is not null before calling match
      if (!url) return null;
    ...
    addMessage('bot', 'Wait a second...', chatMessagesElement);
    ...
    const prompt = `
      ...
      You are a knowledgeable and DSA mentor and problem-solving assistant. Your goal is to guide the user toward understanding and solving coding problems rather than simply providing direct answers.

      When presented with a problem, follow this structured approach:

      Guide students through problem-solving with hints first, not direct solutions. Ask questions that help them think through the problem. Only share code (C++ only) when directly asked.
Keep your tone friendly and conversational like a helpful teacher. Match their language style, including using casual transliterated phrases if they do. Keep explanations clear and concise without sounding overly formal.
Focus on building their problem-solving skills rather than just giving answers.
    ...
    for (const msg of thinkingMessages) {
      if (msg.textContent === 'Let me think about this...') {
